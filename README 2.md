...
1. **Schema-level** — does the BCV table have the columns this mapping says it should, with the types this mapping says it should? The BCV schema is read via `DESCRIBE` over `bcv_analyzer.py`'s connection (not GE's native SQLAlchemy engine — the Presto gateway rejects that engine's `EXECUTE IMMEDIATE`/prepared-statement connection probe). Column **existence** is then asserted as a GE Expectation Suite run against an in-memory pandas batch built from that schema; column **types** are checked directly against the `DESCRIBE` metadata (GE's pandas type inference can't validate Trino type strings like `array(bigint)`). Both are generated from `config/<table>.yaml`'s confirmed-matching and known-type-diff lists (see [`ge_validator/schema_suite.py`](GE_Validation/ge_validator/schema_suite.py)).
