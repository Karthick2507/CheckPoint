...
- Dimensions analyzed: 90 — ✓ pass
- Metrics analyzed: 3 — ✓ pass
- Row count: Control 3,513,794 / Stage 3,513,794 — ✓ match
- Row hashes — ✓ all match

All checks match. As suspected the problem is the bit\_flag column (known diff with BIT\_FLAG\_FORECAST\_EXCLUDE). Once this is fixed, we can re-introduce back into the validations framework and run another round of validations.
...
- Dimensions analyzed: 91 — differences found
- Metrics analyzed: 3 — ✓ pass
- Row count: Control 5,040,770 / Stage 5,040,750 — mismatch
- Row hash diffs: 2,210 — mismatch

## Dimension diffs

bit\_flag

Upon further inspection, we can see that Hoover++ sometimes applies or doesn’t apply the forecast exclude flag. 
