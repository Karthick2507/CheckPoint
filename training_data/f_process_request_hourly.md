...
No such list exists; diffs are KNOWN DIFFERENCES.

---

# 2026-07-21 → Hour 08

Before this run, we can see that we were seeing a `COLUMN LEVEL DIFF` 

Suspicion was correct. Due to post-bid IVT changes, column level diffs put rows in different buckets.

Removing `bit_flag`, `traffic_type`, `ivt_indicator`, `client_facing_ivt_reason_flag`columns, we can see that ALL things match. 

When post-bid IVT validation happens, we can come back and re-introduce these back into the validations framework.

---

# 2026-07-18 → Hour 08

Dimensions and metric values are matching.. We need to figure out why the row count + column level diffs occur. 

Suspicion: `bit_flag`, `traffic_type`, `ivt_indicator` and `client_facing_ivt_reason_flag` column (essentially any IVT related columns)

---

# Problematic Dimensions Fixes

Please check the tracker here: [Discrepancy Tracker](https://freewheel.atlassian.net/wiki/spaces/Infrastructure/pages/191814893/Discrepancy+Tracker)

Validations tracker → [Hoover Validations Documentation Tracker](https://freewheel.atlassian.net/wiki/spaces/Infrastructure/pages/191887907/Hoover+Validations+Documentation+Tracker)
...
Talking to @Li, Ruonan , this is EXPECTED since the IVT Compaction pipeline is NOT running yet.
...
No results. Need to check with @Li, Ruonan or @Li, Anran if maybe a missing `bit_flag`  in `IVTHelper`
...
Old hoover pipeline: In current logic, we put it into event\_hour 20260317190000, it seems correct, even though it’s different with the Flusher split batch. @Gao, Peng can explain more.
