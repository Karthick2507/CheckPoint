...
| request\_\_bid\_request\_\_site\_domain | varchar | YES | used in LQS(40) - this is for inbound bid request (Bidding\_Context.Bid\_Request), not the outbound, outbound bid request is stored in RTB\_Auction  source field in binlog: raw.request.bidding\_context.bid\_request.site.domain |
...
| **Column Name** | **SRC Type** | **BCV Type** | **Changes?** | **Comment** |
| --- | --- | --- | --- | --- |
| request\_\_timestamp | timestamp(3) | timestamp(3) with time zone | YES | all timestamp in the logs are assumed to be in UTC and thus we should convert the bigint timestamp to timestamp(3) without zone infoBCV change is required |
| execution\_networks\_\_network\_selection\_info\_\_candidate\_ad\_funnel\_metrics\_\_phase\_metrics\_\_value | array(array(array(bigint))) | array(array(array(integer))) | YES | is this a typo in creating the views?  the field is defined as uint32 in protobuf ([https://github.freewheel.tv/core/common/blob/master/src/server/protobuf/Log\_Record.proto#L739](https://github.freewheel.tv/core/common/blob/master/src/server/protobuf/Log_Record.proto#L739))  so should be defined as bigint instead of int (H++ schema change) |
...
| **Column Name** | **SRC Values** | **BCV Values** | **Changes?** | **Comment** |
| --- | --- | --- | --- | --- |
| `request__timestamp` | `2026-06-27 18:39:52` | `2026-06-27 14:39:52-04:00` | YES | change to use timestamp(3) in BCV |
...
| `request__client_facing_ivt_reason_flag` | *(null)**(null)* | 0`1125899906842628` | NO | default to 0L in H++   - is this expected? |
| `visitor__identity_user_ids__namespace_id` | `[6]` | *(null)* | YES | , can you check the implementation of `applyPrivacyStripping`()? |
| `visitor__identity_user_ids__id` | `['4f34587bc35290f1ea1be0874bf9ed8db97a50d2']` | *(null)* | YES | same as above |
| `inventory__site_section_chain__tracked_audience_item_ids` | `[[598055, 1150753, 1154115]]` | `[None]` | YES | logic diff, need to be checked and fixed |
...
| `audiences__network_id` | - | - | NO | same as above - only populate when there is a  audience\_item\_ids or kv\_term\_ids populated |
...
| `request__traffic_type` | 0 | 2 | NO | due to lacking of postbid IVT - |
